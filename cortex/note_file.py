...
their order and a new shell appends at the end. Writes take an exclusive flock
...
def render_file(sections: list[tuple[str, str, str | None]]) -> str:
    """[(shell, body, sid)] -> full file text."""
    blocks = [f"{heading(sh, sid)}\n{body}".rstrip()
              for sh, body, sid in sections if body]
    return "\n\n".join(blocks) + "\n" if blocks else ""
    ...
    try:
        ...
        for s, b in split_sections(existing):
            if s == shell:
                ...
                sections.append((s, b, _sid_of(existing, s)))
        if not seen:
            sections.append((shell, body, sid))
