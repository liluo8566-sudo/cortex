...
- lie_down (lie_down.py): next_wake_min REQUIRED at MCP/CLI, clamped lie_down.clamp_next_wake_minutes to [0, wake.next_wake_max] (240) at EVERY hour — 0 = immediate re-wake; proxy callers (fuse/stale) may pass None for `[wake].default_sleep_min`.
- ct-pause = pure stop: `lie_down(book_alarm=False)` books NO next wake (ledger cleared). Manual ct-wake is the only resume and fires a round immediately. fuse/stale still book the default.
